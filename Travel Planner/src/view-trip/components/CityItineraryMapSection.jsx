...
export default function CityItineraryMapSection({ trip, reloadToken = 0 }) {
  ...
  const transportContext = cityMapState.cityMap?.transportContext ?? null;
  const nearestAirports = Array.isArray(transportContext?.nearestAirports)
    ? transportContext.nearestAirports
    : [];
  const nearestStations = Array.isArray(transportContext?.nearestStations)
    ? transportContext.nearestStations
    : [];
  const recommendedArrivalHub = transportContext?.recommendedArrivalHub ?? null;
        ...
        {transportContext ? (
          <div className="rounded-[1.75rem] border border-[var(--voy-border)] bg-[rgba(251,250,247,0.9)] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.6)] md:p-6">
            <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-xs uppercase tracking-[0.22em] text-[var(--voy-text-faint)]">
                  Supplemental transport context
                </p>
                <h3 className="mt-2 text-xl font-semibold text-[var(--voy-text)]">
                  Nearby arrival and local transit references
                </h3>
                <p className="mt-2 max-w-3xl text-sm leading-7 text-[var(--voy-text-muted)]">
                  This block comes from the repo-owned transport dataset and stays
                  separate from itinerary pins. It helps you see the closest airports,
                  stations, and terminals around the same destination shown on the map.
                </p>
              </div>

              <div className="flex flex-wrap items-center gap-3 text-sm text-[var(--voy-text-muted)]">
                <span className="rounded-full border border-[var(--voy-border)] bg-white/80 px-4 py-2">
                  Dataset {transportContext.sourceVersion}
                </span>
                <span className="rounded-full border border-[var(--voy-border)] bg-white/80 px-4 py-2">
                  {transportContext.availableCounts?.airports ?? 0} airports
                </span>
                <span className="rounded-full border border-[var(--voy-border)] bg-white/80 px-4 py-2">
                  {transportContext.availableCounts?.stations ?? 0} stations
                </span>
                {(transportContext.availableCounts?.flightRoutes ?? 0) > 0 ? (
                  <span className="rounded-full border border-[var(--voy-border)] bg-white/80 px-4 py-2">
                    {transportContext.availableCounts.flightRoutes} air links
                  </span>
                ) : null}
              </div>
            </div>

            <div className="mt-5 grid gap-4 xl:grid-cols-[1.15fr,1fr,1fr]">
              <div className="rounded-[1.3rem] border border-[var(--voy-border)] bg-white/80 p-4">
                <p className="text-[11px] uppercase tracking-[0.22em] text-[var(--voy-text-faint)]">
                  Recommended arrival hub
                </p>
                {recommendedArrivalHub ? (
                  <button
                    type="button"
                    className="mt-3 w-full rounded-[1rem] border border-[rgba(24,39,75,0.08)] bg-[rgba(255,255,255,0.86)] px-4 py-4 text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--voy-accent)]"
                    onClick={() => openGoogleMaps(recommendedArrivalHub.mapsUrl)}
                  >
                    <p className="text-base font-semibold text-[var(--voy-text)]">
                      {recommendedArrivalHub.name}
                    </p>
                    <p className="mt-1 text-sm text-[var(--voy-text-muted)]">
                      {recommendedArrivalHub.transportTypeLabel}
                      {recommendedArrivalHub.code
                        ? ` • ${recommendedArrivalHub.code}`
                        : ""}
                    </p>
                    <p className="mt-2 flex items-center gap-2 text-sm text-[var(--voy-text-muted)]">
                      <MapPin className="h-4 w-4" />
                      {recommendedArrivalHub.address || destination}
                    </p>
                    <p className="mt-3 text-sm font-medium text-[var(--voy-text)]">
                      {recommendedArrivalHub.distanceLabel
                        ? `${recommendedArrivalHub.distanceLabel} from the mapped trip center`
                        : "Open in Google Maps"}
                    </p>
                  </button>
                ) : (
                  <div className="mt-3 rounded-[1rem] border border-[var(--voy-border)] bg-white/80 px-4 py-4 text-sm leading-7 text-[var(--voy-text-muted)]">
                    No high-confidence arrival hub was found for this destination in
                    the current transport dataset.
                  </div>
                )}
              </div>
                  ...
                  {nearestAirports.length > 0 ? (
                    nearestAirports.map((airport) => (
                      <button
                        key={airport.id}
                        type="button"
                        className="w-full rounded-[1rem] border border-[rgba(24,39,75,0.08)] bg-[rgba(255,255,255,0.86)] px-3.5 py-3 text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--voy-accent)]"
                        onClick={() => openGoogleMaps(airport.mapsUrl)}
                      >
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="text-xs font-medium text-[var(--voy-text-muted)]">
                            {airport.distanceLabel || "Maps"}
                          </span>
                        </div>
                        <div className="mt-1 text-xs uppercase tracking-[0.12em] text-[var(--voy-text-faint)]">
                          {airport.code || airport.transportTypeLabel}
                        </div>
                        <div className="mt-2 text-sm leading-6 text-[var(--voy-text-muted)]">
                          {airport.address || destination}
                        </div>
                      </button>
                    ))
                  ) : (
                    <div className="rounded-[1rem] border border-[var(--voy-border)] bg-white/80 px-4 py-4 text-sm leading-7 text-[var(--voy-text-muted)]">
                      No airport matches are available for this destination in the
                      supplemental transport dataset.
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="rounded-[1.3rem] border border-[var(--voy-border)] bg-white/80 p-4">
                <p className="text-[11px] uppercase tracking-[0.22em] text-[var(--voy-text-faint)]">
                  Nearest stations and terminals
                </p>
                <div className="mt-3 space-y-2.5">
                  {nearestStations.length > 0 ? (
                    nearestStations.map((station) => (
                      <button
                        ...
                        type="button"
                        className="w-full rounded-[1rem] border border-[rgba(24,39,75,0.08)] bg-[rgba(255,255,255,0.86)] px-3.5 py-3 text-left transition hover:-translate-y-0.5 hover:shadow-sm focus:outline-none focus:ring-2 focus:ring-[var(--voy-accent)]"
                        onClick={() => openGoogleMaps(station.mapsUrl)}
                      >
                        <div className="flex items-start justify-between gap-3">
                          <span className="font-semibold text-[var(--voy-text)]">
                            {station.name}
                          ...
                          <span className="text-xs font-medium text-[var(--voy-text-muted)]">
                            {station.distanceLabel || "Maps"}
                          </span>
                        </div>
                        <div className="mt-1 text-xs uppercase tracking-[0.12em] text-[var(--voy-text-faint)]">
                          {station.transportTypeLabel}
                    ...
                    <div className="rounded-[1rem] border border-[var(--voy-border)] bg-white/80 px-4 py-4 text-sm leading-7 text-[var(--voy-text-muted)]">
                      No local station or terminal matches are available for this
                      destination in the supplemental transport dataset.
                    </div>
                  )}
                </div>
              </div>
